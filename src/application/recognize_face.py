...
class RecognizeFaceUseCase:
    def __init__(
        self, repo: FaceRepositoryFS, camera: CameraService, tolerance: float = 0.5
    ):
        self.repo = repo
        self.camera = camera
        self.tolerance = tolerance

    def execute_from_camera(self) -> Optional[dict]:
        ...
        if dists[idx] <= self.tolerance:
            return {"id": ids[idx], "name": names[idx], "distance": float(dists[idx])}

        return None

    def execute_from_file(self, file_path: str) -> Optional[dict]:
